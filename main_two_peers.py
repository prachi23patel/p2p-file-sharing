...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://peer-to-peer-file-sharing-yvei.vercel.app","http://localhost:3000",         # Local development
...
class ConnectionManager:
    ...
    async def join_room(self, websocket: WebSocket, room_id: str, password: str) -> bool:
        ...
        # Add peer to room
        self.room["peers"].append(websocket)
        self.client_room[websocket] = room_id
        self.client_joined_at[websocket] = datetime.now()
        
        print(f"✅ Peer joined room {room_id}. Total peers: {len(room['peers'])}")
        
        # sending msg to joiner's browser
        await websocket.send_json({
            "type": "joined",
            "room_id": room_id,
        ...
        # it wont work cz there is no peer connection established before we send this offer
        ...
        # sending msg to owner's browser
        owner = room["owner"]
